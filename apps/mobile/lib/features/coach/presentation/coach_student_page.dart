                ...
                children: [
                  ...
                  AppCard(
                    ...
                    child: AppListTile(
                      ...
                      onTap: () => Navigator.of(context).push<void>(
                        MaterialPageRoute(
                          builder: (_) => PerformanceHistoryPage(
                            student: student,
                            batchId: batchId,
                          ),
                        ),
                  ...
                  AppCard(
                    ...
                    child: AppListTile(
                      ...
                      subtitle: const Text(
                        'Attendance + performance summary, generated by AI',
                      ),
                      onTap: () => Navigator.of(context).push<void>(
                        MaterialPageRoute(
...
/// Archetype-C entity hero: a sport-colored gradient band with a back button,
/// the (gated) message-parent action, a large gradient avatar, the student's
/// name in white, and a row of glass chips (sport · skill · Unpaid).
class _Hero extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Deterministic accent from the sport so each sport reads consistently;
    // fall back to navy when the student has no sport.
    final hasSport = sportLabel != '—';
    final colors = hasSport
        ? <Color>[
            colorFromName(sportLabel).withValues(alpha: 0.85),
            colorFromName(sportLabel),
          ]
        : AppPalette.navyGradient;
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              // Adopts the ambient IconTheme so the chat glyph reads white on
              // the hero. Hides itself when no parent is linked.
              IconTheme(
                data: const IconThemeData(color: Colors.white),
                child: MessageParentButton(studentId: student.id),
              ),
            ],
          ),
          const SizedBox(height: AppSpacing.md),
          AppAvatar(student.fullName, size: 72),
          const SizedBox(height: AppSpacing.md),
          Text(
            student.fullName,
          ...
          Wrap(
            ...
            children: [
              if (hasSport)
                AppGlassChip(sportLabel, icon: sportIcon(sportLabel)),
              if (student.skillLevel != null)
                AppGlassChip(student.skillLevel!, icon: Icons.star_rounded),
              // Unpaid (overdue fees) — coaches can't see finance, but this
              // derived flag lets them know the student isn't in good standing.
...
/// Contact card: a divided block of tappable contact rows (parent + student
/// phone/email) that launch the dialer or mail client. Falls back to the
/// parent name when no reachable contact is on file.
class _ContactCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (contacts.isEmpty && student.parentName.isEmpty) {
...
/// "Photos & videos" for one student, presented as a section header + card.
/// Coaches AND trainers can attach standalone media here (no scored
/// assessment) — it's shared straight to the student and their parent. Insert
/// is gated client-side by `uploadStudentMedia` and authoritatively by
      ...
      children: [
        AppSectionHeader(
          ...
          trailing: mediaAsync.maybeWhen(
            orElse: () => null,
            data: (media) =>
                media.isEmpty ? null : AppBadge(text: '${media.length}'),
          ),
        ),
        AppCard(
          child: Column(
            ...
            children: [
              ...
              mediaAsync.when(
                ...
                error: (e, _) => AppErrorView(
                  message: friendlyError(e),
                  onRetry: () => ref
                      .invalidate(mediaForStudentProvider(widget.student.id)),
                ),
                data: (media) {
                  if (media.isEmpty) {
                    return Text(
