...
- 👤 `OW`/`AD` owner+admin — **done** (home dashboard + full billing + team/account + comms/reports/support revamped + verified)

> **2026-06-07 — owner/admin surface workflow + finances fix.** 20 screens revamped/verified (run wf_64f85bbb-ae7, 41 agents, 17 clean / 2 fixed / 1 issue → **resolved**, cohesive:true). Archetype held: `home_tab` + the 5 billing tab-children are app-bar-less (no double bar under the owner shell / billing TabBar); pushed pages/sheets keep their bars; `support_page` thread brought up to the super-tickets pattern. **Issue resolved manually (finances correctness):** the revamp's by-status tap-through on `financial_reports_page` shared `invoicesProvider` (filter-scoped) with the Invoices list, so tapping a report row re-scoped/corrupted the report and the report misrepresented academy-wide finances whenever the invoice filter ≠ All. Fixed by adding an unfiltered academy-wide **`allInvoicesProvider`** in `billing_providers.dart` and pointing the report at it (decoupled; tap-through still pivots the Invoices tab). Whole-app `flutter analyze` = 0 errors. **Minor polish open (not bugs):** unify billing list count affordance (label vs section-header); report tap-through should reset the Money tab's segment to Invoices; `support_page`/`team_page` FABs are ungated (pre-existing — `manageTeam` exists; gate in a follow-up).
